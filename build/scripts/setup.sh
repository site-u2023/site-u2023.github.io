...
[ -n "\${enable_netopt}" ] && {sed -i '$i C=/etc/sysctl.d/99-net-opt.conf; M=\$(awk '\''/MemTotal/{print int(\$2/1024)}'\'' /proc/meminfo); P=\$(grep -c ^processor /proc/cpuinfo); if [ \$M -ge 3072 ]; then R=16777216 W=16777216 TR="4096 262144 16777216" TW="4096 262144 16777216" CT=262144 NB=5000 SC=16384; elif [ \$M -ge 1536 ]; then R=8388608 W=8388608 TR="4096 131072 8388608" TW="4096 131072 8388608" CT=131072 NB=2500 SC=8192; elif [ \$M -ge 512 ]; then R=4194304 W=4194304 TR="4096 65536 4194304" TW="4096 65536 4194304" CT=65536 NB=1000 SC=4096; else exit; fi; [ \$P -gt 4 ] && NB=\$((NB*2)) SC=\$((SC*2)) || [ \$P -gt 2 ] && NB=\$((NB*3/2)) SC=\$((SC*3/2)); printf "net.core.rmem_max=%s\\\\nnet.core.wmem_max=%s\\\\nnet.ipv4.tcp_rmem=%s\\\\nnet.ipv4.tcp_wmem=%s\\\\nnet.ipv4.tcp_congestion_control=cubic\\\\nnet.ipv4.tcp_fastopen=3\\\\nnet.netfilter.nf_conntrack_max=%s\\\\nnet.core.netdev_max_backlog=%s\\\\nnet.core.somaxconn=%s\\\\n" "\$R" "\$W" "\$TR" "\$TW" "\$CT" "\$NB" "\$SC" > \$C; sysctl -p \$C; sed -i '\''/C=\\/etc\\/sysctl.d\\/99-net-opt.conf/d'\'' /etc/rc.local' /etc/rc.local}# BEGIN_CUSTOM_COMMANDS
# END_CUSTOM_COMMANDS
