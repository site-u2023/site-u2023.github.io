...
const PACKAGE_DB = {
  categories: [
    ...
    },
    {
      id: "utilities",
      name: "Utilities",
      description: "System upgrade, VPN and media server utilities",
      packages: [
        {
        ...
        {
          id: "luci-app-dockerman (iptables)",
...
function generatePackageSelector() {
   ...
   // 利用可能パッケージ名のセットを作成（デバイス＋OSバージョンで取得済みのもの）
...
function updatePackageListFromSelector() {
    ...
    syncTemplateAndPackages({ trigger: 'selector' }); // updatedPackages不要
}
...
// ★パッケージインデックス取得（SNAPSHOT/Release 共通）
...
function createExtraPackageBox() {
    ...
    newInput.spellcheck = false;
    newInput.autocapitalize = 'off';
    newInput.dataset.extra = '1'; // 追加欄フラグ

    // イベント紐付け（元と同じ動き＋完全一致優先ソート）
    let timer;
