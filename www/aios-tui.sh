...
generate_files() {
    ...
    if [ -n "$language" ]; then
        var_defs="${var_defs}LANG_PACKAGES=\"luci-i18n-base-${language} luci-i18n-firewall-${language}\"\n"
    fi
...
$var_defs" "$OUTPUT_DIR/postinst.sh.tmp" | sed '/^# BEGIN_VARIABLE_DEFINITIONS/,/^# END_VARIABLE_DEFINITIONS/{/^# BEGIN_VARIABLE_DEFINITIONS/!{/^# END_VARIABLE_DEFINITIONS/!d;}}' > "$OUTPUT_DIR/postinst.sh"
    ...
    rm -f "$OUTPUT_DIR/postinst.sh.tmp"
    
    # Generate setup.sh (existing logic)
    {
        echo "#!/bin/sh"
