...
show_msgbox() {
    ...
    whiptail --title "$breadcrumb" --ok-button "$ok_btn" --msgbox "$message" $WHIPTAIL_HEIGHT $WHIPTAIL_WIDTH
}
...
generate_files() {
    ...
    # Generate postinst.sh
    {
        ...
        echo ""
        echo "# BEGIN_VARIABLE_DEFINITIONS"
        
        all_pkgs=""
        
        if [ -n "$language" ]; then
            all_pkgs="luci-i18n-base-${language} luci-i18n-firewall-${language}"
        fi
        ...
        echo "# END_VARIABLE_DEFINITIONS"
        echo ""
        
        wget -q -O - "$POSTINST_TEMPLATE_URL" | sed '1,/^# END_VARIABLE_DEFINITIONS/d'
    ...
    # Generate setup.sh
    {
        ...
        wget -q -O - "$SETUP_TEMPLATE_URL" | sed '1,/^# END_VARS/d'
    } > "$OUTPUT_DIR/setup.sh"
