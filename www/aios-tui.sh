            ...
            case "$choice" in
                1)
                    if [ -s "$SELECTED_PACKAGES" ]; then
                        ...
                        local pkg_count=$(wc -l < "$SELECTED_PACKAGES")
                        whiptail --scrolltext --title "Package List ($pkg_count packages)" --textbox /tmp/pkg_view.txt 24 78 </dev/tty
                    else
                ...
                2)
                    if [ -s "$SETUP_VARS" ]; then
                        local var_count=$(wc -l < "$SETUP_VARS")
                        whiptail --scrolltext --title "Configuration Variables ($var_count variables)" --textbox "$SETUP_VARS" 24 78 </dev/tty
                    else
                ...
                3)
                    if [ -f "$OUTPUT_DIR/postinst" ]; then
                        whiptail --scrolltext --title "/tmp/postinst" --textbox "$OUTPUT_DIR/postinst" 24 78 </dev/tty
                    else
                ...
                4)
                    if [ -f "$OUTPUT_DIR/setup.sh" ]; then
                        whiptail --scrolltext --title "/tmp/setup.sh" --textbox "$OUTPUT_DIR/setup.sh" 24 78 </dev/tty
                    else
