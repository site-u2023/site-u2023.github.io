...
package_selection() {
    ...
    # ファイル内容をメモリに読み込み
    local memory_selection=""
    if [ -f "$target_file" ]; then
        memory_selection=$(cat "$target_file")
    fi
    
    # メモリ上のenableVar管理用
    local memory_enablevars=""
    if [ -f "$SETUP_VARS" ]; then
        memory_enablevars=$(cat "$SETUP_VARS")
    fi
...
"
                ...
                # メモリ上のデータから選択状態をチェック
                if echo "$memory_selection" | grep -q "=${pkg_name}=" || \
                   echo "$memory_selection" | grep -q "=${pkg_name}\$"; then
                    status="ON"
                else
                    status="OFF"
                fi
...
EOF
        ...
        if [ $? -ne 0 ]; then
            # キャンセル時はメモリ上の変更を破棄してファイルに書き戻さない
            return 0
        fi
        
        # このカテゴリの既存エントリをメモリ上から削除
        local temp_selection=""
        while read -r line; do
            [ -z "$line" ] && continue
            local line_pkg_id=$(echo "$line" | cut -d= -f1)
            local found=0
            
            while read -r pkg_id; do
                [ -z "$pkg_id" ] && continue
                if [ "$line_pkg_id" = "$pkg_id" ]; then
                    found=1
                    break
                fi
            done <<PKGS
...
PKGS
            ...
            if [ "$found" -eq 0 ]; then
                temp_selection="${temp_selection}${line}
"
            fi
        done <<LINES
$memory_selection
LINES
        memory_selection="$temp_selection"
        
        # このカテゴリのenableVarもメモリ上から削除
        while read -r pkg_id; do
            [ -z "$pkg_id" ] && continue
            
            local all_entries
            all_entries=$(echo "$memory_selection" | grep "^${pkg_id}=")
            ...
            while read -r entry; do
                ...
                if [ -n "$enable_var" ]; then
                    memory_enablevars=$(echo "$memory_enablevars" | grep -v "^${enable_var}=")
                    echo "[DEBUG] Removed enableVar from memory: ${enable_var}" >> "$CONFIG_DIR/debug.log" 2>/dev/null
                fi
            done <<ENTRIES
$all_entries
ENTRIES
...
PKGLIST
        ...
        for idx_str in $selected; do
            ...
            if [ -n "$selected_line" ]; then
                ...
                if [ -n "$cache_line" ]; then
                    memory_selection="${memory_selection}${cache_line}
"
                    ...
                    if [ -n "$enable_var" ]; then
                        if ! echo "$memory_enablevars" | grep -q "^${enable_var}="; then
                            memory_enablevars="${memory_enablevars}${enable_var}='1'
"
                            echo "[DEBUG] Added enableVar to memory: $enable_var" >> "$CONFIG_DIR/debug.log" 2>/dev/null
                        fi
                    fi
                fi
            fi
        ...
        # メモリ上の変更をファイルに書き戻し
        echo "$memory_selection" > "$target_file"
        echo "$memory_enablevars" > "$SETUP_VARS"
        
        # キャッシュをクリア
        clear_selection_cache
        
        # 再度メニューを表示するため、ループを継続
    done
}
