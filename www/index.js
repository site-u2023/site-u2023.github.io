...
function resolveChannel(version) {
    return version === 'SNAPSHOT' ? 'snapshotBare'
...
function cacheFrequentlyUsedElements() {
    ...
    state.dom.uciDefaultsContent = document.querySelector('#custom-scripts-details #uci-defaults-content');
}
...
function evaluateInitialPackages() {
    ...
    if (state.apiInfo) {
        console.log('API detected connection type:', getConnectionType(state.apiInfo));
    }
...
function updateVariableDefinitions() {
    const textarea = state.dom.uciDefaultsContent || document.querySelector("#custom-scripts-details #uci-defaults-content");
    if (!textarea) return;
...
function loadUciDefaultsTemplate() {
    const textarea = state.dom.uciDefaultsContent || document.querySelector("#custom-scripts-details #uci-defaults-content");
    if (textarea && !state.dom.uciDefaultsContent) {
        state.dom.uciDefaultsContent = textarea;
    }
...
async function insertExtendedInfo() {
...
async function initializeCustomFeatures() {
    ...
    // Settings bar / custom HTML は index2.html に直書き済み
    setupImportExport();

    cacheFrequentlyUsedElements();

    // autoConfig を1回だけ事前ロード（insertExtendedInfo / fetchAndDisplayIspInfo 双方が参照）
    if (!state.autoConfig) {
        await loadAutoConfig();
    }

    // API fetch / Extended Info DOM生成 / パッケージマネージャ設定を並列実行
    const extendedInfoTask = !document.querySelector('#extended-build-info')
        ? insertExtendedInfo()
        : Promise.resolve();
    ...
    const textarea = state.dom.textarea;
    ...
    // ASUステータスチェックは他をブロックしない（fire-and-forget）
...
console.log('Integrated build fully loaded and ready');
