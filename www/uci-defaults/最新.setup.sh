...
cp \$F \$F.o 2>/dev/null
sed -i '14aDONT_SNAT_TO="0"' \$F
sed -i '/^	  else$/,/^	  fi$/{/^	  else$/a\\
L=0;A="";for P in \$(eval "echo \\\$RULE_\${k}_PORTSETS");do S=\$(echo \$P|cut -d- -f1);E=\$(echo \$P|cut -d- -f2);for x in \$(seq \$S \$E);do A="\$A \$L:\$x,";L=\$((L+1));done;done;A=\${A%?};nft add table inet mape 2>/dev/null;nft add chain inet mape srcnat {type nat hook postrouting priority 0\\;policy accept\\;}2>/dev/null;for p in icmp tcp udp;do nft add rule inet mape srcnat ip protocol \$p oifname map-\$cfg snat ip to \$(eval "echo \\\$RULE_\${k}_IPV4ADDR"):numgen inc mod \$L map {\$A};done
/for portset.*PORTSETS/,/done$/d}' \$F
}
[ -n "\${ap_ip_address}" ] && {
uci -q batch <<AP_EOF
set network.wan.disabled='1'
