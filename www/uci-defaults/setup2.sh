...
{ [ "${connection_type}" = "dslite" ] || { [ "${connection_type}" = "auto" ] && [ "${connection_auto}" = "dslite" ]; }; } && [ -n "${dslite_peeraddr}" ] && {
    ...
    SET ${DSL6}.device="${WAN}" 2>&- || SET ${DSL6}.ifname="${WAN}"
...
{ [ "${connection_type}" = "mape" ] || { [ "${connection_type}" = "auto" ] && [ "${connection_auto}" = "mape" ]; }; } && [ -n "${peeraddr}" ] && {
    ...
    SET ${MAPE6}.device="${WAN}" 2>&- || SET ${MAPE6}.ifname="${WAN}"
...
[ "${connection_type}" = "ap" ] && [ -n "${ap_ipaddr}" ] && [ -n "${gateway}" ] && {
    disable_wan
    {
        ...
        SET ${AP}=interface
        SET ${AP}.proto='static'
        SET ${AP}.device="${LAN}" 2>&- || SET ${AP}.ifname="${LAN}"
        ...
        SET ${AP6}.device="@${AP}" 2>&- || SET ${AP6}.ifname="@${AP}"
