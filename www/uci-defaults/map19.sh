...
while [ $rule -le $units ] ; do
    mark=`expr $rule + 16`
    pn=`expr $rule - 1`
    portl=`expr $rule \* $port_multiplier + $MAPE_PSID \* 16`
    portr=`expr $portl + 15`
    ...
    iptables -t nat -A POSTROUTING -p icmp -m connlimit --connlimit-daddr --connlimit-upto 16 --connlimit-mask 0 -o $TUNDEV -j SNAT --to $IPV4:$portl-$portr
    iptables -t nat -A POSTROUTING -p tcp -o $TUNDEV -m mark --mark $mark -j SNAT --to $IPV4:$portl-$portr
    iptables -t nat -A POSTROUTING -p udp -m connlimit --connlimit-daddr --connlimit-upto 16 --connlimit-mask 0 -o $TUNDEV -j SNAT --to $IPV4:$portl-$portr
...
logger -t nichibann "適用完了: $MAPE_IF IP=$IPV4 PSID=$MAPE_PSID"
EOF
