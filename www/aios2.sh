...
CHECK
            ...
            if [ "$force_remove" -eq 1 ] || [ "$has_other_match" -eq 0 ]; then
                # ★★★ 修正: owner=auto のエントリのみを削除（owner=system は保持）★★★
                if grep -q "^${pkg_id}=.*=auto$" "$SELECTED_PACKAGES" 2>/dev/null; then
                    sed -i "/^${pkg_id}=.*=auto$/d" "$SELECTED_PACKAGES"
                    echo "[AUTO] Removed package: $pkg_id (owner=auto, no matching conditions)" >> "$CONFIG_DIR/debug.log"
                    
                    # enableVar削除（owner=auto の場合のみ）
                    local enable_var
                    enable_var=$(get_package_enablevar "$pkg_id" "")
                    if [ -n "$enable_var" ]; then
                        sed -i "/^${enable_var}=/d" "$SETUP_VARS"
                    ...
                    echo "[DEBUG] Skipped removal: $pkg_id (owner!=auto or not in SELECTED_PACKAGES)" >> "$CONFIG_DIR/debug.log"
                fi
            fi
        fi
    done <<EOF
