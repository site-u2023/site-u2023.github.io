...
`NO_YAML`が設定されていない場合、管理者パスワードはbcryptアルゴリズムでハッシュ化される。この処理のために`htpasswd`コマンドが必要となり、スクリプトは依存パッケージとして`apache`を一時的にインストールする。

処理手順は以下の通りである。

1. 依存パッケージ(`libaprutil`、`libapr`、`libexpat`、`libuuid1`、`apache`)をインストール
2. `/usr/bin/htpasswd`を`/tmp/htpasswd`へ退避
3. `apache`パッケージのみを強制削除
4. `/tmp/htpasswd`を`/usr/bin/htpasswd`へ復元
5. 実行権限を付与

`htpasswd`のインストールに失敗した場合、エラーメッセージを表示して処理を中断する。
...
削除モードは`-r`オプションまたは`REMOVE_MODE`環境変数により指定される。

### autoモード

全ての確認プロンプトをスキップし、設定ファイルも自動的に削除される。インストールエラー時の自動削除処理でも使用される。処理完了後の再起動プロンプトは表示されない。

### manualモード

削除実行前と設定ファイル削除前の2回、ユーザーへ確認を求める。確認に対して`y`または`Y`以外が入力された場合、処理をキャンセルする。

### 未指定時の動作

`REMOVE_MODE`が設定されていない場合、スタンドアロンモードでの実行時のみ、処理完了後に再起動プロンプトが表示される。Enterキー押下により即座に`reboot`コマンドが実行される。

削除処理では、サービスの停止、無効化、パッケージまたはバイナリの削除、設定ファイルの削除（確認後またはautoモード時）、バックアップからの復元、ファイアウォールルールの削除、関連サービスの再起動が順次実行される。remove_adguardhome関数はREMOVE_MODEが未設定の場合、処理完了後に再起動プロンプトを表示する。ただし、adguardhome_main関数からの呼び出し時にはstandalone_mode変数の判定により、統合モード実行時には再起動プロンプトがスキップされる。
...
`http://${NET_ADDR}:${WEB_PORT}/`の形式で表示される。`WEB_PORT`はYAML設定ファイルから読み取られ、設定ファイルが存在しない場合は環境変数`WEB_PORT`のデフォルト値が使用される。
...
`NO_YAML`が設定されていない場合、管理者ユーザー名(`AGH_USER`)とパスワード(`AGH_PASS`)が表示される。`NO_YAML=1`の場合、Webインターフェースでの初期設定が必要となるため、認証情報は表示されない。

### QRコード生成

`qrencode`コマンドが利用可能な環境では、IPv4およびIPv6のアクセスURLに対応するQRコードが自動生成される。QRコードはUTF-8形式でターミナルに直接表示され、モバイルデバイスからのアクセスを容易にする。
