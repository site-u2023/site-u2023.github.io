...
# Note: Default values are set here for non-interactive mode.
# In interactive mode (standalone), prompt_credentials will ask for input.
AGH_USER="${AGH_USER:-}"
AGH_PASS="${AGH_PASS:-}"
  ...
  case "$PACKAGE_MANAGER" in
    opkg)
      ...
      if opkg list-installed 2>/dev/null | grep -q "^apache "; then
        ...
        printf "Apache package detected\n"
      fi
      
      # Check if htpasswd exists after dependency installation
      if [ ! -x /usr/bin/htpasswd ]; then
        ...
        if [ -x /usr/bin/htpasswd ]; then
          ...
          # Only remove apache if it wasn't installed before
...
prompt_credentials() {
  ...
  # In interactive mode (standalone), always prompt for credentials
  # In non-interactive mode, use existing values or defaults
  if [ "$interactive_mode" = "1" ]; then
    ...
    # Non-interactive mode: use existing values or defaults
    [ -z "$AGH_USER" ] && AGH_USER="admin"
    [ -z "$AGH_PASS" ] && AGH_PASS="password"
  fi
...
adguardhome_main() {
  ...
  # Determine if we need interactive credential prompt
  # Interactive mode: standalone execution without pre-set credentials
  if [ -n "$standalone_mode" ] && [ -z "$AGH_USER" ] && [ -z "$AGH_PASS" ]; then
    prompt_credentials "1"  # Interactive mode
  elif [ -z "$AGH_USER" ] || [ -z "$AGH_PASS" ]; then
    prompt_credentials "0"  # Non-interactive mode with defaults
  else
    # Both AGH_USER and AGH_PASS are already set (from environment)
    printf "\033[1;34mUsing pre-configured credentials\033[0m\n"
    printf "Confirm password: "
    stty -echo 2>/dev/null
    read -r AGH_PASS_CONFIRM
    stty echo 2>/dev/null
