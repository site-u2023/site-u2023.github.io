...
スクリプトはインストール時に`/etc/config/`配下の設定ファイルを自動的にバックアップする。バックアップ対象は`dhcp`、`firewall`の2ファイルであり、それぞれ`.adguard.bak`拡張子を付与した形で保存される。
...
他のスクリプトからの統合実行用モードである。処理完了後の再起動プロンプトが抑制される。
