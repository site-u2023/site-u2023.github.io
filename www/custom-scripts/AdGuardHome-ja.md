...
### Webインターフェースのバインドアドレス

公式OpenWrtのAdGuard Homeドキュメントでは、管理Webインターフェースを特定のIPアドレス（例：`192.168.1.1`）にバインドするよう指示していますが、この方法には以下の制限があります：

- IPv6クライアントからWebインターフェースにアクセスできない
- 単一のインターフェースに限定される

本スクリプトはYAML設定テンプレートで`0.0.0.0:{{WEB_PORT}}`を使用しており、利用可能なすべてのネットワークインターフェースにバインドします。これにより以下が可能になります：
- IPv4アドレスからのアクセス
- IPv6アドレスからのアクセス
- ルーターの全インターフェースからのアクセス

これはマルチプロトコル環境における推奨設定であり、ファイアウォールによってLANゾーンに制限されているため、セキュリティを損なうことなく優れたアクセシビリティを提供します。
