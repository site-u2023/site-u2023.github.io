...
function handleTerminalChange() {
    ...
    localStorage.setItem('site-u-terminal', terminalSelector.value);
    updateTerminalCommand();
}

// ── 全リンク更新処理（従来版ベース + 新機能統合） ──
function updateAll() {
    const ipInput = document.getElementById('ip-input');
    const serviceSelector = document.getElementById('service-selector');
    const portInput = document.getElementById('port-input');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (terminalSelector) {
        const storedTerminal = localStorage.getItem('site-u-terminal');
        if (storedTerminal) {
            terminalSelector.value = storedTerminal;
        } else {
    ...
    if (terminalSelector) {
        terminalSelector.addEventListener('change', handleTerminalChange);
    }
    ...
    const terminalUpdateBtn = document.getElementById('terminal-update');
    if (terminalUpdateBtn) {
        terminalUpdateBtn.addEventListener('click', updateTerminalCommand);
    }
